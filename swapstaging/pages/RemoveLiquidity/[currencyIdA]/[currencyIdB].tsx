...
export default function RemoveLiquidity() {
  ...
  const [currencyA, currencyB] = [useCurrency(currencyId1) ?? undefined, useCurrency(currencyId2) ?? undefined]
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        history.push(`/RemoveLiquidity/${currencyId(currency)}/${currencyIdA}`)
      } else {
        history.push(`/RemoveLiquidity/${currencyId(currency)}/${currencyIdB}`)
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        history.push(`/RemoveLiquidity/${currencyIdB}/${currencyId(currency)}`)
      } else {
        history.push(`/RemoveLiquidity/${currencyIdA}/${currencyId(currency)}`)
      }
    },
    [currencyIdA, currencyIdB, history]
  )
                          ...
                          <AutoColumn gap="10px">
                            ...
                            {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                              <RowBetween style={{ justifyContent: 'flex-end' }}>
                                {oneCurrencyIsETH ? (
                                  <StyledInternalLink
                                    href={`/RemoveLiquidity/${currencyA === ETHER ? WETH[chainId].address : currencyIdA}/${currencyB === ETHER ? WETH[chainId].address : currencyIdB
                                      }`}
                                  >
                                    {'Receive WBNB'}
                                  </StyledInternalLink>
                                  ...
                                  <StyledInternalLink
                                    href={`/RemoveLiquidity/${currencyA && currencyEquals(currencyA, WETH[chainId]) ? 'BNB' : currencyIdA
